 ...
 * - Design a `GeoPoint` class which has the field `lat` and `lon` (should be already converted into the rectangular coordinate) as well as `mmRoadId`
...
bool MapMatcher::MapMatching_kernel(list<GeoPoint*>& traj, double candidateRangeM) {
	...
	while (ptIter != traj.end()) {
        ...
        if (candidates_traj.size() == traj.size()) candidates_traj_iter++;
